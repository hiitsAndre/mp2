...
# -*- coding: utf-8 -*-
"""
Created on Sat Mar 10 20:36:38 2018
...
def Heuristic(node):  #Heuristic calculate the remained part of widgets
    result = len(node[1])+len(node[2])+len(node[3])+len(node[4])+len(node[5])
    result = result/5
    return result
...
def Generator(n):   #This will generate 5 random widgets with length n
    global Widget1,Widget2,Widget3,Widget4,Widget5,start
    factory = ['A','B','C','D','E']
    Widget1 = []
    Widget2 = []
    Widget3 = []
    ...
    w5 = ''
    for i in range(n):
        Widget1.append(random.choice(factory))
        Widget2.append(random.choice(factory))
        Widget3.append(random.choice(factory))
        Widget4.append(random.choice(factory))
        Widget5.append(random.choice(factory))
    for i in range(n):
        w1 = w1 + Widget1[i]
        w2 = w2 + Widget2[i]
        w3 = w3 + Widget3[i]
        w4 = w4 + Widget4[i]
        w5 = w5 + Widget5[i]
    start = ('S',w1,w2,w3,w4,w5)
    print(start)
        
Generator(8)#change the input number to generate widgets of different length
